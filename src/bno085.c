...
int i2c_read(sh2_Hal_t * self, uint8_t * buffer, unsigned len,
			  uint32_t * t_us);
...
{ 
	...
	for (int n = 0; n < _prodIds.numEntries; n++)
	{						// sh2.h line 60
...
{ 
	...
	if (x) printf("imu was reset\n");		  
}

		  
 
inline bool hal_reset(void) 
{ 
	// Figure 1-27: 1 – reset
	...
	for (int i = 0;
															   ...
															   i++) 
	{
		auto rc = i2c_write_blocking(i2cX, BNO085_I2C_ADDR, 
...
inline void hal_callback(void *cookie,
						sh2_AsyncEvent_t *
						pEvent) 
{ 
...
inline void sensorHandler(*cookie,sh2_SensorEvent_t *event) 
{
	sensor_event= *event;
	status =sh2_decodeSensorEvent(&sensor_value,&sensor_event);				  
	if (status!=SH2_OK)
	{
...
inline int i2c_read(uint8_t * buffer, unsigned len, uint32_t * t_us) 
{ 
	*t_us = to_us_since_boot(get_absolute_time()); 
	uint8_t shtp_header[4];	// DS: 1.3.1 SHTP
	rc =i2c_read_blocking(i2cX,BNO085_I2C_ADDR,shtp_header,4,false);											  
	if (rc!=4) 
	{ 
...
{															  
	...
	rc=i2c_write_blocking(i2cX,BNO085_I2C_ADDR,buffer,length, false);
...
inline uint32_t getTimeUs(void) 
{ 
